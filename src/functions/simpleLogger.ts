...
export function createSimpleLogger(args: CreateSimpleLoggerArgs): SimpleLogger {
        ...
        if (args.prefix) {
            if (typeof args.prefix === 'string') {
                ...
                const logTypePrefixTable =
                    logTypePrefixes[args.prefix.logTypePrefix || 'NONE'];

                const usingLogTypePrefix = logTypePrefixTable[logType];

                return usingLogTypePrefix
                    ? `[${usingLogTypePrefix}|${args.prefix.simpleString}]: `
                    : `[${args.prefix.simpleString}]: `;
    ...
    return {
        ...
        error: basicLogWrapper(
            args.logFunctions?.error || console.error,
            'error'
        ),
        info: basicLogWrapper(args.logFunctions?.info || console.info, 'info'),
        debug: basicLogWrapper(
            args.logFunctions?.debug || console.debug,
            'debug'
        ),
    };
}
